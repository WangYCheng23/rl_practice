...
LastEditTime: 2024-05-27 10:30:28
'''
import os
...
# class Dog:
p.connect(p.GUI)
plane = p.loadURDF(os.path.join(__file__, "../data/plane.urdf"))
p.setGravity(0,0,-9.8)
p.setTimeStep(1./500)
#p.setDefaultContactERP(0)
#urdfFlags = p.URDF_USE_SELF_COLLISION+p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
urdfFlags = p.URDF_USE_SELF_COLLISION
quadruped = p.loadURDF(os.path.join(__file__, "../data/aliengo/urdf/aliengo.urdf"),[0,0,0.48],[0,0,0,1], flags = urdfFlags,useFixedBase=False)
...
while(1):
    with open(os.path.join(__file__, "../mocap.txt"),"r") as filestream:
